...
st.markdown("""
This app helps learners discover relevant online courses using two smart recommendation methods:

- **Content-Based Filtering**: Suggests courses similar to one you liked, based on features like difficulty, duration, and feedback.
- **Collaborative Filtering**: Recommends top-rated courses you haven’t taken yet, based on what others liked.

Choose a method below to get started!
""")

option = st.radio("Choose recommendation type:", ["Content-Based", "Collaborative", "Evaluate Accuracy"])
...
if option == "Content-Based":
    ...
    if st.button("Run Accuracy Evaluation"):
        rmse, mae = evaluate_accuracy()
        st.success(f"📊 RMSE: {rmse:.3f} | MAE: {mae:.3f}")
